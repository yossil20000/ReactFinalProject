...
function ActionButtons({ OnAction, show, item = "", display ,color,disable}: IActionButtonsProps) {
  ...
  return (
    <>
      {show.includes(EAction.ADD) == true && <Button fullWidth disabled= {getDisable(EAction.ADD ,false)} key={"Add"} variant='outlined' onClick={(event) => OnAction(EAction.ADD, undefined, item)} color={getColor(EAction.ADD,'success')} startIcon={<AddCircleOutlineIcon />}>{getName(EAction.ADD,"Add")}</Button>}
      ...
      {show.includes(EAction.ORDER) == true && <Button fullWidth disabled= {getDisable(EAction.ORDER,false)} key={"Order"} variant='outlined' onClick={(event) => OnAction(EAction.ORDER, undefined, item)} color={getColor(EAction.ORDER,'primary')} startIcon={<AddShoppingCartIcon />}>{getName(EAction.ORDER,"Order")}</Button>}
